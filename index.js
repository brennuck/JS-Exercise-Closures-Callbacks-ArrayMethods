...
function processSum(numberList, callback) {
  return callback(numberList.reduce((accumulator, currentValue) =>
...
function getFullNames(runners) {
  ...
  runners.forEach(function(items){
    roster.push(`${items.last_name}, ${items.first_name}`)
  })
  return roster;
}
...
function firstNamesAllCaps(runners) {
  let roster = runners.map(function(element) {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let roster = runners.filter(function(item) {
...
function tallyUpDonations(runners) {
  let roster = runners.reduce(function(acc, currentValue) {
